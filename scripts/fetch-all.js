...
if (hasGoogle) {
  ...
  try {
    execSync('node scripts/fetch-google.js', { stdio: 'inherit', cwd: ROOT });
  } catch (e) {
    console.error(chalk.red('Google fetcher failed'));
  }
} else {
...
if (hasTA) {
  ...
  try {
    execSync('node scripts/fetch-tripadvisor.js', { stdio: 'inherit', cwd: ROOT });
  } catch (e) {
    console.error(chalk.red('TripAdvisor fetcher failed'));
  }
} else {
